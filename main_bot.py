...
@app.route('/')
def home():
    return "OK - AdivinoPicks Bot running"
...
@app.route('/test')
def test_run():
    """Fuerza la ejecucion del bot ahora mismo para probar"""
    t = threading.Thread(target=run_daily, daemon=True)
    t.start()
    return "✅ Bot ejecutando analisis AHORA. Revisa los logs de Render y tu Telegram.", 200

def start_web_server():
    print(f"[RENDER] Servidor Flask activo en 0.0.0.0:{PORT}")
...
def auto_ping_local():
    while True:
        ...
        try:
            ...
            print(f"[PING] Auto-ping OK - {datetime.now(CARACAS).strftime('%H:%M:%S')}")
        except Exception as e:
            print(f"[PING] Auto-ping fallo: {e}")
...
def api_request(endpoint, params=None):
    ...
    if api_calls >= MAX_API_CALLS:
        print(f"[WARN] Limite API alcanzado ({api_calls}/{MAX_API_CALLS})")
        return []
...
def analizar_partido(match):
    ...
    if api_calls >= MAX_API_CALLS - 5:
        print(f"[WARN] Casi en limite API ({api_calls}/{MAX_API_CALLS})")
        return None
